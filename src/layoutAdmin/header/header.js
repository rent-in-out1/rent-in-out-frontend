import React from 'react'
import { useState } from 'react'
import { Link } from 'react-router-dom'
import { FaSearch, FaUsers, FaListAlt, FaUpload, FaBell,FaHome } from "react-icons/fa"
import { Logo, Wrapper } from '../../components/style/wrappers/navbarAdmin';
const Header = () => {
  const [isLogin, setIsLogin] = useState(true);
  return (
    <Wrapper>
      <div className="container">
        <div className='left flex flex-wrap'>
          <Link to={"/admin"}>
            <Logo>
              <img src="./img/LOGO.png" alt="logo" />
              <p>rentInOut</p>
            </Logo>
          </Link>
          <div className="search">
            <input type="text" placeholder='Search...' className='border-transparent focus:border-transparent focus:ring-0' />
            <div className="icon">
              <FaSearch />
            </div>
          </div>
        </div>
        <div className='right'>
          <nav>
            <ul>

            <li>
                <Link to={"/admin"} class="inline-flex items-center p-3 text-sm text-center">
                  <FaHome className='text-large' />
                </Link>
              </li>
            <li>

                <Link to={"/admin/users"} class="inline-flex items-center p-3 text-sm text-center">
                  <FaUsers className='text-large' />
                </Link>
              </li>
            <li>
                <Link to={"/admin/categories"} class="inline-flex items-center p-3 text-sm text-center">
                  <FaListAlt className='text-large' />
                </Link>
              </li>
              <li>
                <Link to={"/admin/posts"} class="inline-flex items-center p-3 text-sm text-center">
                  <FaUpload className='text-large' />
                </Link>
              </li>
              <li>
                <button type="button" class="inline-flex relative items-center p-3 text-sm  text-center">
                  <FaBell />
                  <span class="sr-only">Notifications</span>
                  <div class="z-10 inline-flex absolute -top-1 -right-2 justify-center items-center w-5 h-5 text-xs font-bold text-white bg-red-500 rounded-full">2</div>
                </button>
              </li>
            </ul>
          </nav>
          <div className="relative avatar">
            <img className="rounded-full" src={isLogin ? "https://images.pexels.com/photos/2385044/pexels-photo-2385044.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" : "data:image/png;base64,"} alt="" />
            <span className={`${isLogin ? "bg-green-400" : "bg-red-400"} bottom-0 left-7 absolute  w-3.5 h-3.5 border-2 border-white dark:border-gray-800 rounded-full`}></span>
          </div>
        </div>
      </div>
    </Wrapper>
  )
}

{/* <div className="relative">
<img className="w-10 h-10 rounded-full" src={isLogin ? "https://images.pexels.com/photos/2385044/pexels-photo-2385044.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" : "data:image/png;base64,"} alt="" />
<span className={`${isLogin ? "bg-green-400" :"bg-red-400"} bottom-0 left-7 absolute  w-3.5 h-3.5 border-2 border-white dark:border-gray-800 rounded-full`}></span>
</div> */}
export default Header