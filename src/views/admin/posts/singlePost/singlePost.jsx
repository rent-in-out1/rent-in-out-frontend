...
const SinglePost = ({ post, setIsChange }) => {
	...
	return (
		<tr>
			<td>
				<div className='flex items-center'>
					<div className='flex-shrink-0 w-8 h-8'>
						<img className='w-full h-full rounded-full' src={post?.img[0]?.url} alt={post?.img} />
					</div>
					<div className='ml-3'>
			...
			<td>
				<p
					onClick={() => nav(`/admin/profile/${post?.creator_id?._id}`)}
				...
				>
					{post?.creator_id?.fullName?.firstName} {post?.creator_id?.fullName?.lastName}
				</p>
			</td>
			<td>
