...
const SingleUser = (props) => {
	...
	return (
		<tr>
			<td>
				<div className='flex items-center'>
					<div className='flex-shrink-0 w-8 h-8'>
						<img className='w-full h-full rounded-full' src={user?.profile_img.url} alt='profile' />
					</div>
					<div className='ml-3'>
			...
			<td>
				<p className='text-gray-900 whitespace-no-wrap'>
					{Math.floor(new Date(Date.now()).getFullYear() - new Date(user?.birthdate).getFullYear())}
				</p>
			</td>
			<td>
			...
			>
				<span className='btn relative inline-block px-3 py-1 font-semibold leading-tight cursor-pointer'>
					<span
						...
						className={
							user?.role === 'admin'
								? 'absolute inset-0 bg-black opacity-50 rounded-full'
