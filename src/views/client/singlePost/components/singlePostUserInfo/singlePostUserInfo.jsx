		...
		<div className='user-header capitalize mb-2 border p-2 relative'>
			...
			<div className=''>
				<ul className='relative leading-8 '>
					...
					<li>active since: {new Date(owner?.createdAt).toLocaleDateString()}</li>
					<li>
						email:{' '}
				...
				<button onClick={() => setIsShow(!show)} className='w-3/12 h-1/2 absolute top-4 right-10 '>
					{show ? (
			...
			<SinglePostUserRating rank={rank} post={post} isChange={isChange} setIsChange={setIsChange} />
		</div>
	);
};
