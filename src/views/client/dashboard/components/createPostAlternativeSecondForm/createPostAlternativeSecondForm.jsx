...
const CreatePostAlternativeSecondForm = ({ data, setData, setDisplay, handleOnChange, setOnAdd, images }) => {
	...
	const handleSelectSuggestion = (suggestion) => {
		...
		if (map) {
			...
			map.setView([y, x], 13);
			const newMarker = L.marker([y, x]).addTo(map).bindPopup(label).openPopup();
			setMarker(newMarker);
		}
		setQuery('');
	};
	...
	const handleUpload = () => {
		if (data.collect_points.length === 0) return errorHandler('You must provide at list one collection point');
		if (data.price === '') return errorHandler('You must provide a price');
		if (data.category_url === '') return errorHandler('You must choose category');
	...
	const handleDeleteAddress = (selectedAddress) => {
		const updatedSelectedAddresses = selectedAddresses.filter((address) => address.label !== selectedAddress.label);
	...
	return (
		<React.Fragment>
			<form className='min-h-min mb-4 capitalize flex flex-col gap-3'>
				<div className='flex gap-2 w-full'>
					<input
						...
						min={0}
					/>
					<select value={data?.category_url ?? 'Choose Category'} name='category_url' onChange={handleOnChange}>
						<option value='Choose Category' className='capitalize'>
							Choose Category
						</option>
						{category?.map((category) => (
							<option value={category?.url_name} key={category.name} className='capitalize'>
								{category.name}
							</option>
						))}
					</select>
				...
				<div className='filters w-full flex gap-2'>
					<input value={data?.country} type='text' name='country' placeholder='Country' onChange={handleOnChange} />
					<input value={data?.city} type='text' name='city' placeholder='City' onChange={handleOnChange} />
				</div>
				...
				<div>
					{selectedAddresses.length > 0 && (
						<>
							<small className='block text-left ml-1'>Selected Collection Points</small>
							<ul className='mb-2 flex flex-col gap-1'>
								{selectedAddresses.map((address) => (
									<div className='flex justify-between border rounded p-2' key={address.label}>
										<li>{address.label}</li>
										<span className='cursor-pointer' onClick={() => handleDeleteAddress(address)}>
											<Minus color='red' />
										</span>
									</div>
								))}
					...
					<div className='relative'>
						...
						{suggestions.length > 0 && (
							<ul className='absolute z-[10000] w-full' style={{ listStyleType: 'none', padding: '0', margin: '0' }}>
			...
			<div className='flex justify-between px-2'>
				{getButtonsProps(setDisplay, closeUploadSection, handleUpload, isDisable).map((buttonsGroup, i) => (
					<div key={i}>
						{buttonsGroup.map(({ handleClick, disabled, className, type, text }) => (
							<button key={text} onClick={handleClick} disabled={disabled} className={className} type={type}>
								{text}
							</button>
						))}
					</div>
