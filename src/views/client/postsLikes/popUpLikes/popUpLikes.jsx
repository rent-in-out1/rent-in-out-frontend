...
const PopUpLikes = ({ likesArr }) => {
	...
	const searchFilterLikesArr = () => {
		...
		setLikes_ar(
			likesArrBackup.filter((solidLike) => {
		...
		<PopUPModel action={onLikesToggle}>
			...
			<form className='px-3 py-2'>
				<div className='relative'>
					...
					<button
						type='button'
						className='text-white absolute right-2.5 bottom-2.5 bg-blue-500 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2'
					>
						Search
					</button>
				</div>
			...
			<ul className='overflow-y-scroll max-h-96 px-3 w-full pb-3'>
				{likes_ar.length ? (
					likes_ar?.map((item) => {
						return <PostsLikesSingle key={item._id} item={item} action={onLikesToggle} />;
					})
				) : (
