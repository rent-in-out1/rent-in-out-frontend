...
const PostHeader = ({ post }) => {
	...
	return (
		<div className='dashboard-post-header-wrapper flex justify-between items-center pr-2 p-1'>
			<div onClick={() => navigateToUserProfile()} className='flex items-center cursor-pointer capitalize'>
				...
				<h5 className='pl-1 flex'>
					...
					<label className='ml-1'>{post?.creator_id.fullName?.lastName}</label>
				</h5>
			</div>
			<div
				className='z-10'
				...
				>
					<li
						...
						className={`transition duration-100 ease-in-out cursor-pointer px-4 py-2 flex justify-between items-center hover:bg-gray-200 ${
							user?._id !== post?.creator_id && 'rounded-b-xl hover:rounded-b-xl'
						}`}
					>
						<p>Share</p>
						<Send />
