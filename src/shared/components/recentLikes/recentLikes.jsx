...
const RecentLikes = ({ likes = [] }) => {
	...
	return (
		<React.Fragment>
			...
			>
				<span className='text-xs font-semibold mr-1 rounded'>{likes.length < 1 || null}</span>
				{likes.length > 0 && (
					<div className='flex items-center justify-between relative'>
						{likes?.slice(0, 3)?.map((like, i) => {
							return (
								<div key={like._id} className={`w-6 h-6 bg-red-200 border rounded-full absolute -top-3 left-${i * 4}`}>
									<img
