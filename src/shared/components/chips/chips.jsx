...
const chipVariants = cva('text-xs rounded-full cursor-pointer transition duration-300 flex items-center', {
	variants: {
		variant: {
...
const Chips = ({ chipsProp, setChipsProp, setForm, showChipsCount = 7, className, variant, size, ...props }) => {
	...
	return (
		<div className='flex flex-wrap gap-1'>
			{arrSlice1 &&
				arrSlice1.map((chip, i) => {
					return (
						<div key={i}>
							{chip.name && (
								<div
									onClick={() => handleChecked(chip._id)}
									data-tooltip-id={`chip-${chip.name}`}
									className={`${cn(chipVariants({ size, variant, className }))} ${chip.check ? 'bg-neutral-200' : ''}`}
									{...props}
								>
									{chip.check && (
										<span className='mr-1'>
							...
							{chip.info && <ReactTooltip id={`chip-${chip.name}`} place='top' variant='dark' content={chip.info} />}
						</div>
					);
				})}
			{/* add more chips */}
			<div
				...
				className={`${cn(chipVariants({ size, variant, className }))} relative inline-block select-none`}
				{...props}
			>
				Choose another ..
				{/* more options */}
				{isOpenMoreOptions && (
					...
					>
						<div className='flex flex-wrap gap-1 m-4 w-56 md:w-72 h-auto max-h-36 md:max-h-48 overflow-y-auto z-50 p-2 rounded-sm bg-white shadow-2xl'>
							{arrSlice2 &&
								arrSlice2.map((chip, i) => {
									return (
										<div key={i}>
											{chip.name && (
												<div
													onClick={() => moveToFirstGroup(chip._id)}
													className={`${cn(chipVariants({ size, variant, className }))} border-black`}
													{...props}
												>
													{chip.check && (
														<span className='mr-1'>
